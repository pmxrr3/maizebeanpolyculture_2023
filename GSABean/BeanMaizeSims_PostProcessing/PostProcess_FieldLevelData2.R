field_poly_1 <- 'C:/Users/Rik/OneDrive - The University of Nottingham/FSPM/Climbing bean data for experiments/field_monoculture_1 - Copy.txt'
...
Mean_Maizedata_poly_1 <- sapply(3:ncol(Maizedata_poly_1), function (j) sapply(1:13, function(i) sapply(1:100, function(x) mean(c(Maizedata_poly_1[(i-1)*300+x, j],Maizedata_poly_1[(i-1)*300+x+100, j],Maizedata_poly_1[(i-1)*300+x+200, j])))))
...
#
Mean_beandata_poly_1 <- sapply(3:ncol(Beandata_poly_1), function (j) sapply(1:13, function(i) sapply(1:100, function(x) mean(c(Beandata_poly_1[(i-1)*300+x, j],Beandata_poly_1[(i-1)*300+x+100, j],Beandata_poly_1[(i-1)*300+x+200, j])))))
...
#
STD_Maizedata_poly_1 <- sapply(3:ncol(Maizedata_poly_1), function (j) sapply(1:13, function(i) sapply(1:100, function(x) sd(c(Maizedata_poly_1[(i-1)*300+x, j],Maizedata_poly_1[(i-1)*300+x+100, j],Maizedata_poly_1[(i-1)*300+x+200, j])))))
...
#
STD_Beandata_poly_1 <- sapply(3:ncol(Beandata_poly_1), function (j) sapply(1:13, function(i) sapply(1:100, function(x) sd(c(Beandata_poly_1[(i-1)*300+x, j],Beandata_poly_1[(i-1)*300+x+100, j],Beandata_poly_1[(i-1)*300+x+200, j])))))
...
###
library(dplyr)
Titlelist <- c('Row distance', 'Max internode length', 'Helix inclination', 'Branching probability', 'Time to flower', 'Plant distance')
columnnumber <- 3
ymax <- 5 #1500
yLabel <- 'LAI' #'Biomass (g/m^2)' #Biomass (g/m^2)
for(counter in 1:length(Titlelist)) {
  legendlabels <- case_when(
    counter==1 ~ c("Maize 0.25m", "Maize 0.5m","Maize 1.0m", "Bean 0.25m", "Bean 0.5m", "Bean 1.0m"),
    ...
    counter==6 ~ c("Maize 0.1m", "Maize 0.2m","Maize 0.4m", "Bean 0.1m", "Bean 0.2m", "Bean 0.4m"),
